...
Modalbox.Methods = {
	...
	options: {
		...
		top: 200, // Default top position, Added by Jakobra
		left: null, // Default left position, null meaning it will display as centered, Added by Jakobra
	...
	_init: function(options) {
		...
		this.MBwindow = new Element("div", {id: "MB_window", style: "display: none"}).update(
			this.MBframe = new Element("div", {id: "MB_frame"})
		);
	...
	resize: function(byWidth, byHeight, options) { // Change size of MB without loading content
		...
		var newHeight = ((w.height + byHeight) < cHeight) ? (cHeight) : (w.height + byHeight);
		...
		if(this.options.transitions) {
			...
			new Effect.Morph(this.MBwindow, {
				style: "width:" + newWidth + "px; height:" + newHeight + "px; left:" + ((oWidth - newWidth)/2),
	...
	_insertContent: function(content, callback){
		...
		if(this.options.height == this._options.height) {
			Modalbox.resize((this.options.width - w.width), $(this.MBcontent).getHeight() - w.height, {
			...
			this._setWidth();
			this.MBcontent.setStyle({overflow: 'auto', height: w.height - 13 + 'px'});
			setTimeout(function(){ // MSIE fix
				this._putContent(callback);
			}.bind(this), 1);
		}
	...
	_setPosition: function () {
		if(this.options.left == null) {
			...
			this.MBwindow.setStyle({left: this.options.left + "px"});
		}
		//Added by Jakobra
		this.MBwindow.setStyle({top: this.options.top + "px"});
	},
